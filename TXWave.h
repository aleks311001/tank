...
//!
//! @see     txWaveSampleRate, txWaveFormat, txWaveBuffer_t, txWaveLoadWav(), txWaveOut(), txWaveIn()
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
//!
//! @see     txWaveSampleRate, txWaveVolMax, txWaveBuffer_t, txWaveFormat, txWaveLoadWav(), txWaveOut(), txWaveIn()
//!
//! @usage @code
//!          // Сирена 880 Гц, формируемая в буфере
//!
//!          int time = 2000;
//!          txWaveBuffer_t buf (ROUND (time * txWaveSampleRate));
//!
...
//!
//! @see     txWaveBuffer_t, txWaveSampleRate, txWaveVolMax, txWaveFormat, txWaveLoadWav(), txWaveOut(), txWaveIn()
//!
//! @usage @code
//!          int time = 2000;
//!          txWaveBuffer_t wav (ROUND (time * txWaveSampleRate));
//!
...
typedef std::vector <txWaveSample_t> txWaveBuffer_t;
...
HWAVEOUT txWaveOut (int timeMs = -INT_MAX,
                    ...
                    int loops = 1,
                    const txWaveBuffer_t& data = txWaveBuffer_t());
...
inline
HWAVEOUT txWaveOut (const txWaveBuffer_t& data, int loops = 1)
    {
...
typedef bool MonitorProc_t (HWAVEIN waveIn, txWaveBuffer_t& data, void* userData);
...
//! @return  Если надо продолжать запись, то @c true. Если надо закончить запись - @c false.
//!
//! @see     txWaveIn(), txWaveGetPosition(), txWaveVolMax, txWaveFormat, txWaveBuffer_t, txWaveSample_t
//!
//! @usage @code
//!          bool MonitorProc (HWAVEIN, txWaveBuffer_t& data, void*)
//!              {
...
bool MonitorProc (HWAVEIN waveIn, txWaveBuffer_t& data, void* userData);
#endif
...
//!
//! @see     monitorProc(), txWaveOut(), txWaveGetPosition(), txWaveSampleRate, txWaveVolMax, txWaveFormat,
//!          txWaveBuffer_t, txWaveSample_t
//!
//! @usage @code
//!          int time = 20000;
//!          txWaveBuffer_t wav = txWaveIn (time, MonitorProc);  // Записываем звук
...
txWaveBuffer_t txWaveIn (int timeMs,
...
//!
//! @return  @ref txWaveBuffer_t, т.е. <tt>std::vector \<@ref txWaveBuffer_t\>,</tt> с загруженными данными
//!
//! @note    Файл должен быть в формате WAV, Microsoft PCM, Стерео (2 канала), скорость семплирования 44.100 кГц
//!          (@ref txWaveSampleRate), 16 бит на канал. Если файл другого формата, его надо преобразовать.
//!
//! @see     txWaveOut(), txWaveIn(), txWaveSampleRate, txWaveVolMax, txWaveFormat
//!
//! @usage @code
//!          txWaveBuffer_t purr = txWaveLoadWav ("Purr.wav");
//!
...
txWaveBuffer_t txWaveLoadWav (const char filename[]);
...
txWaveBuffer_t txWaveIn (int timeMs,
    ...
    data.resize (size);
    txWaveBuffer_t (data) .swap (data);  // C++11: data.shrink_to_fit();
    data.resize (0);
...
struct WAVFILEHEADER
    ...
    #ifdef _GCC_VER                // MSVC doesn't allow zero-sized arrays in the middle of structs
    ...
    unsigned long  subchunk2Size;  // numSamples * pcm.wf.nChannels * pcm.wBitsPerSample/8

    txWaveSample_t buffer[0];      // data [numSamples], size in bytes = subchunk2Size
    };
...
txWaveBuffer_t txWaveLoadWav (const char filename[])
    ...
    TRY
        ...
        try { buffer.resize (size / sizeof (txWaveSample_t)); }
        catch (...) { FAILURE; }

        // Read the file header

        WAVFILEHEADER* wav = (WAVFILEHEADER*) &buffer[0];
        ...
        // Skip metadata
            ...
            unsigned metaSize = (wav->subchunk1Size == sizeof (PCMWAVEFORMAT) + 2)? wav->metadata[0].size : 0;
        ...
        buffer.resize (size / sizeof (txWaveSample_t));
        }
