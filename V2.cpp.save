...
#include "TXNetwork.h"
#include "TXLib.h"
#define SERVER_IP "localhost"  //IP ñåðâåðà  //10.0.2.205
...
int main ()
    ...
    for (int i = 0, t_p = 0; !GetAsyncKeyState (VK_ESCAPE) && tank[0].XP > 0; i ++, t_p ++, tank[0].XP += 0.0025)
        ...
        txRecvFrom (client, &Per_Data_1, sizeof (Per_Data_1));              //      | \_____/  |
        txSendTo   (client, &Per_sn_0,   sizeof (Per_sn_0));                //      | | ÊÎÒ |  |
        txRecvFrom (client, &Per_sn_1,   sizeof (Per_sn_1));                //      | | ÊÎÒ |  |
        //txSendTo   (client, &Per_Obj_s,  sizeof (Per_Obj_s));             //      | |_____|  |
